...
public class PostControllerTest {
    ...
    void 사용자는_게시물을_단건_조회_할_수_있다() throws Exception {
        ...
        // when
        // then
        mockMvc.perform(get("/api/v0/posts/2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").isNumber())
                .andExpect(jsonPath("$.content").value("hellowWorld"))
                .andExpect(jsonPath("$.writer.id").isNumber())
                .andExpect(jsonPath("$.writer.email").value("kok202@naver.com"))
                .andExpect(jsonPath("$.writer.nickname").value("test202"));
    }
    ...
    void 사용자가_존재하지_않는_게시물을_조회할_경우_에러가_난다() throws Exception {
        ...
        // when
        // then
        mockMvc.perform(get("/api/v0/posts/123456789"))
                .andExpect(status().isNotFound())
                .andExpect(content().string("Post에서 ID 123456789를 찾을 수 없습니다."));
    }
